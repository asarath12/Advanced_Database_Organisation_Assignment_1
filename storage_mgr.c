...

{
    ...
    //This block of code was to create small individual page files.  We are appending blocks
    //into the existing "file".The page file shall be created with an automatically generated name based on
    //its page number and original name.
    //The file needs to be linked other pages.
    //Testing for beginning Total Page Number
    char* pt1_filename=strdup(fHandle->fileName); // copying filename for base of small files, remove .XXX
    pt1_filename[strlen(pt1_filename)-4] = 0; // delete last three characters
    printf("this is the reduced file name: %s\n", pt1_filename);
    //get a temp name for the new file.
    int newTotal= fHandle->totalNumPages;
    newTotal++;//number for update
    printf("this is the int page number part of file name: %d\n", newTotal);
    char pt2_filename [100];
    sprintf(pt2_filename, "%d", newTotal);//convert to char
    //int num_digits = snprintf(NULL, 0, "%d", newTotal); //convert to a char*

    //char* pt2_filename = malloc((num_digits + 1) * sizeof(char));

    printf("this is the page number part of file name: %s\n", pt2_filename);
    size_t len1 = strlen(pt1_filename);
    size_t len2 = strlen(pt2_filename);
    size_t total_len = len1 + len2 + 1;

    char* temp_filename = (char*)malloc(total_len * sizeof(char));
    if (temp_filename == NULL) {
        return RC_APPEND_FAILED;
    }

    strcpy(temp_filename, pt1_filename);
    strcat(temp_filename, pt2_filename);
    // add ".bin" to name
    strcat(temp_filename, ".bin");
    //sprintf(temp_filename, "file_%d.bin", newTotal);


    printf("New file name: %s\n", temp_filename);
    //use createPageFile to add to overall File in next project iteration
    if (RC_OK == createPageFile(temp_filename)) {
